...
import com.company.entities.animals.Sex;
...
public abstract class Animal extends BaseEntity implements CanEat, Spawn, CanMove {
    ...
    protected int maxPopulationToSpawn;


    @Override
    public void eat(int howMany, String animal) {
        fullness += howMany;
        if (fullness >= needAMeal)
            fullness = needAMeal;
    }
    ...
    public int getMaxPopulationToSpawn() {

        return maxPopulationToSpawn;
    }

    public void lostEnergy() {
        if (fullness < needAMeal / 3) {
            fullness -= 2;
        }
        else if (fullness < 1) {
            health--;
    ...
    @Override
    public boolean spawn(int howMany, Animal animal) {
